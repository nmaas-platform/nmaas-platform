            ...
            map(apps => {
                return apps
                    .map(app => {
                        ...
                        app.versions = app.versions.filter(appVersion => (typeof appVersion.state === 'string') ? appVersion.state !== 'DELETED' : appVersion.state !== ApplicationState.DELETED)
                        return app
                    })
                    // filter out deleted apps or apps with no versions
                    .filter(app => ((typeof app.state === 'string') ? app.state !== 'DELETED' : app.state !== ApplicationState.DELETED) && app.versions.length >= 1)
