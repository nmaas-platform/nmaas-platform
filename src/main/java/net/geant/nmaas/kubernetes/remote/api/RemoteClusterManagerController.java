...
public class RemoteClusterManagerController {
    ...
    @GetMapping("/all")
    public List<RemoteKClusterDto> getAllRemoteClusters() {
        return remoteClusterManager.getAllClusters();
    }

    @GetMapping("/base")
    public List<RemoteKClusterBaseDto> getAllRemoteClustersBaseInfo() {
        return remoteClusterManager.getAllClustersBase();
    }
    ...
    @PreAuthorize("hasRole('ROLE_SYSTEM_ADMIN') || hasRole('ROLE_OPERATOR') || hasRole('ROLE_DOMAIN_ADMIN')")
    @PutMapping("/{id}")
    public RemoteKClusterDto updateKubernetesCluster(@PathVariable Long id, @RequestBody RemoteKClusterCompleteDto dto) {
        return remoteClusterManager.updateCluster(dto, id);
    }
