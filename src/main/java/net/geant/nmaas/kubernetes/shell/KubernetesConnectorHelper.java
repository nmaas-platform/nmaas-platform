...
public class KubernetesConnectorHelper {
    ...
    public Map<String, String> getPodNamesForAppInstance(Long appInstanceId) {
        ...
        return getPodNamesForAppInstance(
                applicationInstanceService.find(appInstanceId).orElseThrow(() -> new RuntimeException("Application instance not found"))
        );
    }
    ...
    private Map<String, String> getPodNamesForAppInstance(AppInstance appInstance) {
        ...
        final String namespace = getTargetNamespace(kCluster, appInstance.getDomain().getCodename());
    ...
    public AsyncConnector preparePodShellConnection(AppInstance appInstance, String podName) {
        ...
        final KCluster kCluster = Objects.nonNull(appDeployment.getRemoteClusterId()) ?
                kClusterRepository.getReferenceById(appDeployment.getRemoteClusterId()) : null;
        final String namespace = getTargetNamespace(kCluster, appInstance.getDomain().getCodename());
        return new PodShellConnector(kubernetesApiClientService.getDirectClient(kCluster), namespace, podName);
    }
