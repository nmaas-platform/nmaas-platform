...
class KubernetesParameterGenerator {
    ...
    static Map<String, String> createAdditionalGlobalParametersMap(Map<String, String> globalDeployParameters, UnaryOperator<String> variableResolver) {
        Map<String, String> additionalParameters = new HashMap<>();
        globalDeployParameters.forEach((k, v) -> additionalParameters.put(k, createParameterValueString(v, additionalParameters, variableResolver)));
        return additionalParameters;
    }

    private static String createParameterValueString(String value, Map<String, String> additionalParameters, UnaryOperator<String> variableResolver) {
        // verify if parameter references a centrally stored variable and resolve it
        if (VariableReference.isReference(value)) {
            String variableName = VariableReference.extractName(value)
                    .orElseThrow(() -> new IllegalArgumentException("Invalid variable reference " + value));
            return variableResolver.apply(variableName);
        }
        return createRandomOrStaticValue(value, additionalParameters);
    }

    private static String createRandomOrStaticValue(String value, Map<String, String> additionalParameters) {
