...
public class DomainObjectPermissionCheck extends BasePermissionCheck {
    ...
    protected void setupMatrix() {
        ...
        globalPermMatrix.put(Role.ROLE_GROUP_DOMAIN_ADMIN, new Permissions[]{Permissions.READ});
        globalPermMatrix.put(Role.ROLE_GROUP_MANAGER, new Permissions[]{Permissions.READ, Permissions.WRITE});
        ...
        permMatrix.put(Role.ROLE_DOMAIN_ADMIN, new Permissions[]{Permissions.READ, Permissions.OWNER});
        permMatrix.put(Role.ROLE_GROUP_DOMAIN_ADMIN, new Permissions[]{Permissions.READ, Permissions.OWNER});
        permMatrix.put(Role.ROLE_GROUP_MANAGER, new Permissions[]{Permissions.READ, Permissions.WRITE});
    ...
    protected Set<Permissions> evaluatePermissions(User user, Serializable targetId, String targetType) {
        ...
        Domain globalDomain = domains.getGlobalDomain().orElseThrow(() -> new IllegalArgumentException("Global domain not found"));
        for (UserRole role : user.getRoles()) {
            if (globalDomain.equals(role.getDomain())) {
                resultPerms.addAll(Arrays.asList(globalPermMatrix.get(role.getRole())));
                if (Role.ROLE_GROUP_MANAGER.equals(role.getRole()) && !globalDomain.equals(domain)) {
                    resultPerms.add(Permissions.OWNER);
                }
