...
public class BulkDomainServiceIntTest {
    ...
    void shouldHandleBulkCreationOfUsersWithSsoEnabledFlagWhenDisabledGlobally() {
        ...
        configurationRepository.save(modelMapper.map(
                ConfigurationView.builder().id(1L).defaultLanguage("en").bulkDomainsAllowForSsoAccounts(false).parallelDeploymentsLimit(2).bulkDeploymentJobCron("* * * *").bulkDeploymentQueueRefresh(60).build(), Configuration.class));
        UserViewMinimal creator = new UserViewMinimal();
        creator.setId(1L);
        creator.setUsername("admin");
