...
for NAME in ../data/apps/*.json; do
    ...
    for IMAGETYPE in data/apps/images/logo/$APPNAME.*; do
            EXTENSION="${IMAGETYPE##*.}"
    ...
    curl --silent -X POST $API_URL/apps --header "Authorization: Bearer $TOKEN" --header "Content-Type: application/json" --header "Accept: application/json" -d @$NAME
		# add logo
		curl --silent -X POST --header "Authorization: Bearer $TOKEN" -F "file=@../data/apps/images/logo/$APPNAME.$EXTENSION;type=$CONTENT_TYPE" $API_URL/apps/$APPID/logo
		# add all screenshots
		for SCREENSHOT in ../data/apps/images/screenshots/$APPNAME/*; do
			curl --silent -X POST --header "Authorization: Bearer $TOKEN" -F "file=@data/apps/images/screenshots/$APPNAME/"${SCREENSHOT##*/}";type=image/png" $API_URL/apps/$APPID/screenshots
		done
		# activate the app "
		curl --silent -X PATCH $API_URL/apps/state/$APPID --header "Authorization: Bearer $TOKEN" --header "Content-Type: application/json" --header "Accept: application/json" -d @data/apps/activations/active.json
	else
