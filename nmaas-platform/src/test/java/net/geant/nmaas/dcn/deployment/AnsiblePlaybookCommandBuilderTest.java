...
public class AnsiblePlaybookCommandBuilderTest {
    ...
    private static final String EXAMPLE_COMPLETE_ANSIBLE_PLAYBOOK_COMMAND_FOR_CLIENT_SIDE_ROUTER =
            "ansible-playbook " +
    ...
    private static final String EXAMPLE_COMPLETE_ANSIBLE_PLAYBOOK_COMMAND_FOR_CLOUD_SIDE_ROUTER =
            "ansible-playbook " +
                    "-v -i /ansible-playbook-dir/hosts " +
    ...
    @Test
    public void shouldBuildCorrectAnsibleCommandForClientSideRouter() {
        ...
        for (String command : commands)
            assertThat(EXAMPLE_COMPLETE_ANSIBLE_PLAYBOOK_COMMAND_FOR_CLIENT_SIDE_ROUTER, Matchers.stringContainsInOrder(Arrays.asList(command)));
    }

    @Test
    public void shouldBuildCorrectAnsibleCommandForCloudSideRouter() {
        final List<String> commands = AnsiblePlaybookCommandBuilder.command(
        ...
        for (String command : commands)
            assertThat(EXAMPLE_COMPLETE_ANSIBLE_PLAYBOOK_COMMAND_FOR_CLOUD_SIDE_ROUTER, Matchers.stringContainsInOrder(Arrays.asList(command)));
    }
