...
insert into service_storage_volume_deploy_parameters(service_storage_volume_id, deploy_parameters, deploy_parameters_key)
  select ssv.id, 'persistence.enabled', 'PERSISTENCE_ENABLED' from service_storage_volume ssv
  union
  select ssv.id, 'persistence.name', 'PERSISTENCE_NAME' from service_storage_volume ssv
  union
  select ssv.id, 'persistence.storageClass', 'PERSISTENCE_STORAGE_CLASS' from service_storage_volume ssv
  union
  select ssv.id, 'persistence.size', 'PERSISTENCE_STORAGE_SPACE' from service_storage_volume ssv;
